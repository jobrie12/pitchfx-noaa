...
cur_time = time.time()
f = open("./Summary Observations - {}.txt".format(int(cur_time)),"w+")
...
"""
masterList = {'games':[], 'game_ids':[], 'years':{}, 'stadiums':[], }
count = [{}]
wonky_records = []
spring_game_count = 0
for i in range1(1,30):
    count.append({'ob':0,'games':0,'game_ids':0, 'years':{}, 'stadiums':0})
    ...
    for index, row in df.iterrows():
        ...
        gameID = row["game_game_pk"]
        if pd.notna(gameID) and gameID != curGameID:
            if not gameID in masterList['game_ids']:
                masterList['game_ids'].append(gameID)
                count[i]['game_ids'] += 1
            else :
                print(gameID)
                print(game, row['game_type'], row['game_venue'],row['stadium_name'],row['team_name'],row['player_team'])
                if pd.isnull(row['game_type']):
                    wonky_records.append(row)
            curGameID = gameID

                
        if pd.notna(game) and game != curGame:
            if row['game_type'] == 'S':
                spring_game_count += 1
                if pd.isnull(gameID):    
                    wonky_records.append(row)
            if game in masterList['games']:
                curGame = game
            else:
                masterList['games'].append(game)
                curGame = game
                try: 
                    count[i]['years'][year] += 1
                except:
                    count[i]['years'][year] = 1
                try:
                    masterList['years'][year] += 1
                except:
                    masterList['years'][year] = 1
                count[i]["games"] += 1
                if not stadium in masterList['stadiums']:
                    masterList['stadiums'].append(stadium)
                    count[i]['stadiums'] += 1
        count[i]["ob"]+= 1
        total += 1
    print("Processed {}:\n{} games {} ids".format(i,count[i]['games'], count[i]['game_ids']))
f.write("Total Observations: {}\n".format(total))
f.write("Total Games: {}\n".format(len(masterList['games'])))
f.write("Total Games by ID: {}\n".format(len(masterList['game_ids'])))
f.write("Total Stadiums: {}\n".format(len(masterList['stadiums'])))
f.write("Games by Years Covered: {}\n".format(masterList['years']))
f.write("\n----------------------------\n")
f.write("Individual File Breakdown")
f.write("\n----------------------------\n")
for i in range1(1,30):
    f.write("\nFile {}.csv\n-------------\n\nObservations: {}\nGames: {}\nGames by ID: {}\nUnique Stadiums Added: {}\n".format(i, count[i]["ob"], count[i]["games"], count[i]["game_ids"], count[i]["stadiums"]))
    f.write("Games by Year : {}\n".format(count[i]['years']))
f.close()